...
static void _cbDialog(WM_MESSAGE * pMsg)
{
    ...
    case WM_INIT_DIALOG:
        ...
        if (clockFormat == 24)
        {
            CreateListWheel(0, 0,  ch, h, GUI_ID_LISTWHEEL0, hours24,
                            ...
                            spinWheel, &dateTimeWheels[0], &GUI_Font32B_ASCII, current_hour);
        }
        else
        {
            int hour;
    ...
    case WM_NOTIFY_PARENT:
        ...
        case ID_BUTTON_SAVE:
            ...
            {
            case WM_NOTIFICATION_RELEASED:
                current_hour = LISTWHEEL_GetPos(dateTimeWheels[0].hWin);
                current_minute = LISTWHEEL_GetPos(dateTimeWheels[1].hWin);
                current_ampm = LISTWHEEL_GetPos(dateTimeWheels[2].hWin);
                current_day = LISTWHEEL_GetPos(dateTimeWheels[3].hWin);
                current_month = LISTWHEEL_GetPos(dateTimeWheels[4].hWin);
                current_year = LISTWHEEL_GetPos(dateTimeWheels[5].hWin) + 2010;
#ifndef CODEBLOCK
                RTC_TimeTypeDef tm;
                RTC_DateTypeDef dt;
